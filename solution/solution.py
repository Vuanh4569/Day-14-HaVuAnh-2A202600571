...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            overlap = len(chunk_tok & exp_tok) / len(exp_tok)
            is_relevant.append(1 if overlap >= relevance_threshold else 0)
...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if scores:
                item_averages.append(sum(scores.values()) / len(scores))
        ...
        if len(item_averages) > 1:
            ...
            if item_averages[0] > rest_avg + 0.15:
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            res = evaluator.run_full_eval(answer, pair.question, pair.context or "", pair.expected_answer)
            res.qa_pair = pair
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        n = len(new_results)
        b = len(baseline_results)
        
        new_avg_f = sum(r.faithfulness for r in new_results) / n if n > 0 else 0.0
        new_avg_r = sum(r.relevance for r in new_results) / n if n > 0 else 0.0
        new_avg_c = sum(r.completeness for r in new_results) / n if n > 0 else 0.0
        
        base_avg_f = sum(r.faithfulness for r in baseline_results) / b if b > 0 else 0.0
        base_avg_r = sum(r.relevance for r in baseline_results) / b if b > 0 else 0.0
        base_avg_c = sum(r.completeness for r in baseline_results) / b if b > 0 else 0.0
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        low_metrics = []
        if f == min_score:
            low_metrics.append('f')
        if r == min_score:
            low_metrics.append('r')
        if c == min_score:
            low_metrics.append('c')
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures, start=1):
            ...
            if idx - 1 < len(suggestions):
                ...
                fix = suggestions[-1] if suggestions else "Review pipeline components"
                
            rows.append(f"| {fid} | {ftype} | {rc} | {fix} | Open |")
    ...
    def generate_improvement_suggestions(
        ...
        for f_type, _ in sorted_failures:
            if f_type.lower() == "hallucination":
                suggestions.append("Implement hallucination checker to filter unsupported claims")
            elif f_type.lower() == "incomplete":
                suggestions.append("Increase chunk size in RAG pipeline to reduce context fragmentation")
            elif f_type.lower() == "irrelevant":
                suggestions.append("Refine system prompt and add direct instructions to align answer with user query")
            elif f_type.lower() == "off_topic":
                suggestions.append("Improve intent routing classifier before sending to core agent")
                
        defaults = [
            "Implement hallucination checker to filter unsupported claims",
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Add few-shot examples showing complete answers to improve completeness",
            "Refine system prompt and add direct instructions to align answer with user query"
        ]
        for d in defaults:
